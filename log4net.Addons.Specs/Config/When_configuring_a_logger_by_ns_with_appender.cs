...
namespace log4net.Addons.Specs.Config
{
    ...
    public class When_configuring_a_logger_by_ns_with_appender
    {
        Because of = () => _log = new FluentConfigurator()
                                      .Logger<FluentConfigurator>()
                                      .All()
                                      .AddAppender(new MemoryAppender())
                                      .Configure();

        It should_not_be_null = () => _log.ShouldNotBeNull();
        It should_have_the_given_type_name = () => _log.Logger.Name.ShouldEqual("log4net.Addons.Config.FluentConfigurator");
        It should_be_configured_for_all_levels = () => _log.GetLogger().Level.ShouldEqual(Level.All);
